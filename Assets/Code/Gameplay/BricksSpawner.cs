...
namespace Gameplay
{
    public sealed class BricksSpawner : MonoBehaviour
    {
        ...
        private void CreateBricksRow(int columnCount, float posY)
        {
            ...
            if (columnCount % 2 != 0)
            {
                Instantiate(brickPrefab, new Vector3(_centerPosX, posY, 0f), Quaternion.identity, transform);
            ...
            for (int i = 0; i < columnCount / 2; i++)
            {
                ...
                CreateTwoMirrorBricks(new Vector3(x, posY, 0f), distanceBetweenBricks);
                distanceBetweenBricks += _brickWidth * 2;
            }
        }

        private void CreateTwoMirrorBricks(Vector3 leftBrickPos, float distanceBetweenBricks)
        {
            Vector3 rightBrickPos = leftBrickPos + new Vector3(distanceBetweenBricks, 0, 0);
            Instantiate(brickPrefab, leftBrickPos, Quaternion.identity, transform);
            Instantiate(brickPrefab, rightBrickPos, Quaternion.identity, transform);
        }
    }
}